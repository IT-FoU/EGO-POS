...
- [x] Implement local POS checkout through `commitLocalAndQueue`: sale, sale items, payments, stock/lot consumption, loyalty event, receipt snapshot, audit event and operation envelope. <!-- Phase 6A: CASH-only atomic commit (sale, items, cash payment, stock/lot consumption, immutable receipt snapshot, audit, operation envelope). Phase 6C wired it into the existing PosPageClient (offlineCheckout prop + offlineCashCheckoutPosPolicy) with outbox flush + reconciliation (checkout/outbox-flush.ts). Loyalty event + non-cash tenders remain out of scope (later phases). See docs/offline/IMPLEMENTATION_PROGRESS.md "Phase 6C". -->
<!-- Phase 6C (approved separately) delivered the offline CASH checkout UI + outbox flush + reconciliation in the EXISTING PosPageClient (no second POS): flag OFF / flag ON+online preserve online checkout exactly; flag ON + unlocked authorized offline-ready terminal offline enables cart add/remove/qty/barcode + CASH pay via commitOfflineCashSale (one operationId/attempt), shows a permanent receipt reference + "Pending sync", clears cart once; non-cash/holds/voids/refunds/returns/cash-movement/stock actions disabled offline; on reconnect flushOutbox pushes ready ops one-at-a-time in dependency order (no parallel, no offline network, same-operationId retry on interrupt), reconciles pending->synced (local->cloud ids) or pending->rejected (immutable receipt kept), then pulls deltas. -->
