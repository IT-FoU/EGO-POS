...
| POS checkout/payment/receipt | `completeSaleAction`, `writeCompletePrismaSale`, `checkout-receipt.ts` | Sales, payments, lots, movements, derived cash-session totals | Complete sale, tender, print, Recent Sales | VERIFIED SOURCE; EGO-FIX-07 isolated 28/28; `/pos` HTTP smoke LOADS; Production write smoke not run; refund/void/exchange not in this phase |
...
Reason: a production target without catalogue, inventory, and Manager/Cashier role tests cannot safely process real Mini Mart operations. Super Admin bootstrap, Production migration targeting, POS inventory-lot reconciliation, local browser smoke, Products first-stock, POS discovery/cart, and isolated checkout/payment/receipt integrity are in place. Refund/void/exchange, promotions, membership, and reports are not independently verified. Owner-created live catalogue is still required before real-store use.
