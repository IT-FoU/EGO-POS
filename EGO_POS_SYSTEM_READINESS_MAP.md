...
Reason: a production target without catalogue, inventory, and Manager/Cashier role tests cannot safely process real Mini Mart operations. Super Admin bootstrap, Production migration targeting, POS inventory-lot reconciliation, local browser smoke, Products first-stock, POS discovery/cart, isolated checkout/payment/receipt, and isolated refund/void/exchange integrity are in place. Promotions, membership, and reports are not independently verified. Owner-created live catalogue is still required before real-store use.
