    ...
    return (<div className="flex min-w-0 flex-col gap-6 overflow-x-hidden">
      <section className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-5 flex items-start gap-4">
          ...
          <div>
            <h1 className="text-3xl font-semibold">Promotion Analytics</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{t("ui.mock.demo.analytics.foundation.final.version")}</p>
          </div>
        </div>
      </section>

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <Metric icon={Gift} label="Total promotion sales" value={`${formatLak(totalSales)} LAK`}/>
        <Metric icon={TrendingDown} label="Total discount given" value={`${formatLak(totalDiscount)} LAK`}/>
        <Metric icon={BarChart3} label="Total usage" value={formatLak(totalUsage)}/>
        <Metric icon={TrendingUp} label="Revenue generated" value={`${formatLak(totalSales)} LAK`}/>
        <Metric icon={TrendingUp} label="Estimated profit" value={`${formatLak(estimatedProfit)} LAK`}/>
        <Metric icon={Users} label="Member vs non-member" value="62% / 38%"/>
      </section>

      <section className="grid gap-6 xl:grid-cols-2">
        <AnalyticsTable title="Top 10 performing promotions" promotions={topPromotions}/>
        <AnalyticsTable title="Worst performing promotions" promotions={worstPromotions}/>
      </section>

      <section className="grid gap-6 xl:grid-cols-3">
        <Panel title="Promotion usage trend" value="Chart placeholder: usage by day/week/month."/>
        <Panel title="Sales by promotion type" value={promotions.map((promotion) => formatPromotionType(promotion.type)).join(", ")}/>
        <Panel title="Margin impact" value={t("ui.final.margin.impact.requires.sale.item.cost.")}/>
      </section>
    </div>);
}
...
function AnalyticsTable({ promotions, title }: {
      ...
      <div className="mt-5 max-w-full overflow-x-auto">
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead className="border-b border-border text-xs uppercase text-muted-foreground">
            <tr><th className="px-3 py-3">Promotion</th><th className="px-3 py-3">Type</th><th className="px-3 py-3 text-right">Usage</th><th className="px-3 py-3 text-right">Revenue</th><th className="px-3 py-3 text-right">Discount</th></tr>
          </thead>
          <tbody>
