...
const TEST_REF = "ieutdqnlfiiaawctapor";
...
function readTestDatabaseUrl() {
  ...
  if (!existsSync(envLocal)) {
    throw new Error("Missing .env.local for TEST DATABASE_URL");
  }
  ...
  if (!databaseUrl.includes(TEST_REF)) throw new Error("Refusing non-TEST database for Hyperdrive local build env");
...
console.log("Set Hyperdrive local build env host=aws-0-ap-southeast-1.pooler.supabase.com TEST_REF=ieutdqnlfiiaawctapor");
