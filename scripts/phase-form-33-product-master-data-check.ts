...
check("12-16. Supplier select (preferred via products.supplier_id) — M2M blocked", () => {
  assert(schema.includes("supplierId"), "products.supplier_id missing");
  assert(!schema.includes("model ProductSupplier") && !schema.includes("product_suppliers"), "unexpected M2M table present");
  assert(productForm.includes("createSupplierAction"), "missing supplier create");
  assert(productForm.includes("supplierId: selectedSupplierId || undefined"), "payload missing supplierId");
  assert(productForm.includes("preferredSupplierHint"), "preferred hint missing");
...
check("17-20. Filter foundation by category/brand/supplier without join duplication", () => {
  ...
  assert(where.supplierId === "sup-1", "supplier filter not applied");
  assert(!("productSuppliers" in where), "unexpected M2M join in where");
});
