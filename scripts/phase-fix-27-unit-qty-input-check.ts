...
check("source: ProductUnitsTable uses qty input handlers, not per-keystroke updateUnit", () => {
  ...
  assert(productForm.includes("qtyInputDisplay"), "display helper missing");
  ...
  assert(productForm.includes("next.draft === undefined && !next.error && next.committed !== previousCommitted"), "blur commit missing");
});
...
check("8. No pricing/cost values change during this input edit", () => {
  const start = [
  ...
  const next = applyUnitPricingPatch({
    editedUnitId: "pack",
    patch: { hierarchyQty: during.committed },
    shareStock: true,
    units: start,
  });
