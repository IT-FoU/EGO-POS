      ...
      <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(320px,420px)_minmax(0,1fr)]">
        <div className="min-w-0 rounded-lg border border-border bg-card p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-semibold">{t("supplierList")}</h2>
            <SupplierStatusBadge locale={locale} status="active" />
          </div>
              ...
              return (
                <button
                  className={
                    selected
                      ...
                      : "rounded-md border border-border p-4 text-left transition hover:border-primary"
                  }
                  key={supplier.id}
                  type="button"
                ...
                >
                  <div className="flex min-w-0 flex-col gap-2">
                    <div className="min-w-0">
                      <div className="truncate font-semibold" title={supplier.name}>
                        {supplier.name}
                      </div>
                  ...
                  <div className="mt-3 text-sm text-muted-foreground">{t("outstanding")}</div>
          ...
          <div className="flex min-w-0 flex-col gap-6">
            <section className="rounded-lg border border-border bg-card p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <SupplierStatusBadge locale={locale} status={selectedSupplier.status} />
              </div>
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                <Metric label={t("creditLimit")} value={formatMoney(selectedSupplier.creditLimitLak, "LAK")} />
                <Metric label={t("outstandingBalance")} value={formatMoney(selectedSupplier.outstandingBalanceLak, "LAK")} />
                <Metric label={t("openOrders")} value={String(supplierOrders.length)} />
              </div>
            ...
            <section className="grid min-w-0 gap-6 lg:grid-cols-2">
              <div className="min-w-0 rounded-lg border border-border bg-card p-5">
                <h3 className="text-lg font-semibold">{t("purchaseOrders")}</h3>
                <div className="mt-4 flex flex-col gap-3">
                  {supplierOrders.map((order) => (
                    <div className="rounded-md border border-border p-3" key={order.id}>
                      <div className="flex items-center justify-between gap-3">
                  ...
                  {supplierOrders.length === 0 ? <p className="text-sm text-muted-foreground">{t("noPurchaseOrders")}</p> : null}
                </div>
              </div>
              <div className="min-w-0 rounded-lg border border-border bg-card p-5">
                <h3 className="text-lg font-semibold">{t("creditSummary")}</h3>
                <div className="mt-4 flex flex-col gap-3">
                  {supplierPayables.map((payable) => (
                    <div className="rounded-md border border-border p-3" key={payable.id}>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-3 text-sm">
                        <Metric label={t("paid")} value={formatMoney(payable.paidAmountLak, "LAK")} />
                        <Metric label={t("unpaid")} value={formatMoney(payable.balanceAmountLak, "LAK")} />
                      </div>
                    </div>
                  ))}
                  {supplierPayables.length === 0 ? <p className="text-sm text-muted-foreground">{t("noSupplierCredit")}</p> : null}
                </div>
              </div>
            </section>
          </div>
