...
function mapPostSaleError(error: unknown, fallback: string) {
  const message = error instanceof Error ? error.message : String(error ?? "");
  const lower = message.toLowerCase();
  if (lower.includes("permission") || lower.includes("forbidden")) {
    return tPos("ui.permission.denied");
  }
  if (lower.includes("manager pin approval failed") || lower.includes("approval failed")) {
    return tPos("ui.invalid.pin");
  }
  if (lower.includes("manager pin") && lower.includes("required")) {
    return tPos("ui.approval.required");
  }
  if (lower.includes("already voided")) {
    return tPos("ui.sale.already.voided");
  }
  ...
  if (!message.trim() || message === "Post-sale request failed.") {
    return tPos("ui.operation.failed");
  }
  // Keep known business-rule messages; never surface stack traces.
  if (message.includes("\n") || message.length > 180) {
    return fallback;
  }
...
export function ReturnExchangeVoidModal({
  currentRole,
  initialSaleId,
  initialTab = "return",
  onBack,
  ...
  const [pendingAction, setPendingAction] = useState<ReturnExchangeTab | null>(null);
  ...
  function openApprovalModal(action: ReturnExchangeTab) {
    const defaultReason =
      reason.trim() ||
      (action === "void"
        ? `${tPos("ui.void.sale")} ${sale?.saleNo ?? ""}`.trim()
        : action === "exchange"
          ? `${tPos("ui.exchange")} ${sale?.saleNo ?? ""}`.trim()
          : `${tPos("ui.return.and.refund")} ${sale?.saleNo ?? ""}`.trim());
    setPendingAction(action);
    setApprovalReason(defaultReason);
    clearApprovalPin();
    setApprovalOpen(true);
    setError(null);
  }

  async function executeAction(action: ReturnExchangeTab, approval?: PostSaleManagerApprovalPayload) {
    if (!sale || submitLockRef.current) return;
    submitLockRef.current = true;
    setBusy(true);
    setError(null);
    try {
      if (action === "return") {
        const result = await returnSaleRequest(
          sale.id,
          {
      ...
      if (action === "exchange") {
        const result = await exchangeSaleRequest(
          sale.id,
          {
      ...
      const fallback =
        action === "void" ? tPos("ui.void.failed") : action === "exchange" ? tPos("ui.exchange.failed") : tPos("ui.return.failed");
      setError(mapPostSaleError(submitError, fallback));
      throw submitError;
    } finally {
      clearApprovalPin();
      setBusy(false);
  ...
  async function requestAction(action: ReturnExchangeTab) {
    if (!sale || busy || submitLockRef.current) return;

    if (action === "return" && selectedReturns.length === 0) {
      setError(tPos("ui.select.returned.item"));
      return;
    }
    if (action === "exchange" && (selectedReturns.length === 0 || replacements.length === 0)) {
    ...
    if (needsPostSaleManagerPin(currentRole, action)) {
    ...
    try {
      ...
      // Error already mapped into UI state.
    }
  }

  async function submitApproval() {
    if (!pendingAction || busy || submitLockRef.current) return;
    const pin = approvalPin.trim();
    const pinReason = approvalReason.trim() || reason.trim();
    if (!pin || !pinReason) {
      setError(tPos("ui.manager.pin.reason.required"));
      return;
    }
    ...
    // Clear PIN from React state before the network round-trip completes.
    clearApprovalPin();
    try {
      await executeAction(pendingAction, approval);
      setApprovalOpen(false);
      setPendingAction(null);
      setApprovalReason("");
    } catch {
      // Keep approval modal open for retry; PIN already cleared.
    }
  }

  const approvalTitle =
    pendingAction === "void"
      ? tPos("ui.manager.approval.void")
      : pendingAction === "exchange"
        ? tPos("ui.manager.approval.exchange")
        : tPos("ui.manager.approval.return");
  const approvalSubmitLabel =
    pendingAction === "void"
      ? tPos("ui.approve.void")
      : pendingAction === "exchange"
        ? tPos("ui.approve.exchange")
        : tPos("ui.approve.return");
  ...
  return (
    <>
                ...
                {!sale ? (
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      {tab === "return" ? (
                        <button className="h-11 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground" disabled={busy} type="button" onClick={() => void requestAction("return")}>
                          {tPos("ui.confirm.return")}
                        </button>
                      ) : null}
                      {tab === "exchange" ? (
                        <button className="h-11 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground" disabled={busy} type="button" onClick={() => void requestAction("exchange")}>
                          {tPos("ui.confirm.exchange")}
                        </button>
                      ) : null}
                      {tab === "void" ? (
                        <button className="h-11 rounded-md bg-danger px-4 text-sm font-semibold text-primary-foreground" disabled={busy} type="button" onClick={() => void requestAction("void")}>
      ...
      {approvalOpen ? (
        ...
        >
          <div className="grid gap-4" data-testid="pos-return-exchange-pin-modal">
            <div className="rounded-lg border border-border bg-background p-3 text-sm">
              <p className="font-mono font-semibold">{sale?.saleNo}</p>
              <p className="text-xs text-muted-foreground">{sale?.receiptNo} · {sale?.customerName}</p>
            </div>
            <label className="grid gap-2 text-sm font-semibold">
              ...
              <textarea
                ...
                placeholder={pendingAction === "void" ? tPos("ui.void.reason") : tPos("ui.refund.reason")}
              />
            </label>
            <label className="grid gap-2 text-sm font-semibold">
              {tPos("ui.manager.owner.pin")}
              <input
                autoComplete="off"
                ...
                placeholder={tPos("ui.enter.manager.pin")}
              />
            </label>
            {error ? <p className="rounded-md border border-danger/40 bg-danger/10 px-3 py-2 text-sm text-danger">{error}</p> : null}
            <div className="flex flex-wrap justify-end gap-2">
              <button className="h-10 rounded-md border border-border px-4 text-sm font-semibold" type="button" onClick={closeApprovalModal}>
                {tPos("ui.cancel")}
              </button>
              <button className="h-10 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground" disabled={busy} type="button" onClick={() => void submitApproval()}>
                {approvalSubmitLabel}
              </button>
            </div>
          </div>
